...
import {
    ...
    Activity, Target, Zap, TrendingUp, ChevronLeft, ChevronRight, LayoutDashboard, Layers
} from 'lucide-react'
...
export default function AdminDashboard() {
    ...
    const [view, setViewInternal] = useState<'analytics' | 'users' | 'validation' | 'regulation' | 'import' | 'prompt' | 'taxonomy'>('analytics')
    ...
    useEffect(() => {
        ...
        const validTabs = ['analytics', 'users', 'validation', 'regulation', 'import', 'prompt', 'taxonomy']
    ...
    const [dbStats, setDbStats] = useState({ total: 0, pending: 0, users: 0 })
    ...
    const fetchRealData = async () => {
        ...
        const { data: pkgs } = await supabase
            ...
            `)
            .in('status', ['processing', 'draft', 'pending', 'published_partially'])
            .order('created_at', { ascending: false })
        
        if (pkgs) setPendingPackages(pkgs)

        // 2. Counts
        const { count: totalQ } = await supabase.from('questao_base').select('*', { count: 'exact', head: true })
        const { count: pendQ } = await supabase.from('package_questions').select('*', { count: 'exact', head: true })
        const { count: totalU } = await supabase.from('users').select('*', { count: 'exact', head: true })
    ...
    const handlePublishPackage = async (pkgId: string) => {
        setIsPublishing(pkgId)
        try {
            const token = localStorage.getItem('sb-czguyzdbvqfyjsfwcpnh-auth-token')
            const access_token = token ? JSON.parse(token).access_token : null

            const res = await fetch(`/api/admin/packages/${pkgId}/publish`, {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                setImportStatus({ type: 'success', msg: `Publicado! ${data.publishedQuestionsCount} questões ativas.` })
                fetchRealData()
                await useTaxonomy.getState().loadTaxonomy()
            } else {
                setImportStatus({ type: 'error', msg: data.error || 'Erro ao publicar' })
            }
        } catch (err) {
            setImportStatus({ type: 'error', msg: 'Erro de rede' })
        } finally {
    ...
    const handleBatchImport = async () => {
        ...
        const mockGeneratedQuestions: Question[] = Array(batchSize).fill(null).map((_, i) => {
            ...
            return {
                ...
                // Master Mapping Fields
                area_id: targetCourse.id || 'CM',
                subarea_id: `${targetCourse.id || 'CM'}-${randomSpec.id.toUpperCase()}`,
                tema_id: `${targetCourse.id || 'CM'}-${randomSpec.id.toUpperCase()}-${chosenSub.id.toUpperCase()}`,
                fonte: 'ia',
        ...
        <div className="space-y-10 relative">
            ...
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 font-medium max-w-2xl text-sm mt-2">
                        Controle total da engine médica, gestão de lotes Revalida e monitoramento de alunos.
                    </p>
                </div>

                <div className="flex items-center gap-3">
                    <button className="px-6 py-4 rounded-2xl bg-white text-slate-400 font-black text-[10px] uppercase tracking-widest border border-slate-100 flex items-center gap-2 hover:text-primary transition-all shadow-sm">
                        <RefreshCw className="w-4 h-4" /> Reset Cache
                    </button>
                    <Link href="/concursos/admin">
                        <button className="px-6 py-4 bg-[#1A1033] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-600/20 hover:scale-105 transition-all flex items-center gap-2">
                            <Zap className="w-4 h-4 text-primary" /> Permutar Concursos
                        </button>
                    </Link>
                </div>
            </header>

            {/* Main Navigation Protocol */}
            <nav className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-4 -mx-4 px-4 lg:mx-0 lg:px-0">
                <TabButton active={view === 'analytics'} onClick={() => setView('analytics')} label="Geral" icon={LayoutDashboard} />
                <TabButton active={view === 'users'} onClick={() => setView('users')} label="Alunos" icon={Users} />
                <TabButton active={view === 'validation'} onClick={() => setView('validation')} icon={CheckCircle2} label="Validação" />
                <TabButton active={view === 'regulation'} onClick={() => setView('regulation')} icon={ShieldCheck} label="Regulação" />
                <TabButton active={view === 'import'} onClick={() => setView('import')} icon={Sparkles} label="Auto Gerador" />
                <TabButton active={view === 'prompt'} onClick={() => setView('prompt')} icon={Target} label="Gerador de Prompt" />
                <TabButton active={view === 'taxonomy'} onClick={() => setView('taxonomy')} icon={Layers} label="Taxonomia" />
            </nav>
                        ...
                        <div className="bg-card border border-border rounded-[40px] p-10 space-y-6 soft-shadow">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50/50 p-8 rounded-[32px] border border-slate-100">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Especialidade Alvo</label>
                                    <select
                                        value={selectedSpecialty}
                                        onChange={(e) => {
                                            setSelectedSpecialty(e.target.value)
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer hover:border-primary/30 transition-all"
                                    >
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Lote de Geração</label>
                                    <select
                                        value={selectedBatchSize}
                                        onChange={(e) => setSelectedBatchSize(parseInt(e.target.value))}
                                        className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer"
                                    >
                                        <option value={500}>Lote Master (500 Questões)</option>
                                        <option value={100}>Lote Padrão (100 Questões)</option>
                                        <option value={50}>Lote Rápido (50 Questões)</option>
                                        <option value={10}>Lote Teste (10 Questões)</option>
                                        <option value={1}>Single Questão</option>
                                    </select>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Nível de Complexidade</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        <option value="Fácil">Básico (Conceitual)</option>
                                        <option value="Médio">Intermediário (Clínica)</option>
                                        <option value="Difícil">Avançado (Especialista)</option>
                                    </select>
                                </div>
                            </div>

                            <div className="relative group">
                                <label className="absolute -top-3 left-6 px-2 bg-card text-[10px] font-black uppercase tracking-widest text-primary z-10">Console de Saída JSON</label>
                                <textarea
                                    value={jsonInput}
                                    onChange={(e) => setJsonInput(e.target.value)}
                                    placeholder='Aguardando comando de geração ou entrada manual...'
                                    className="w-full h-48 bg-slate-900 border border-slate-800 rounded-[24px] p-8 font-mono text-xs text-emerald-400 focus:ring-2 focus:ring-primary/20 outline-none resize-none shadow-inner"
                                />
                                <div className="absolute bottom-4 right-4 flex gap-2">
                                    <button onClick={handleDownloadBackup} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl text-slate-400 transition-all" title="Download Backup">
                                        <Database className="w-4 h-4" />
                                    </button>
                                </div>
                            </div>
                            ...
                            {importStatus && (
                                <motion.div 
                                    initial={{ opacity: 0, height: 0 }}
                                    animate={{ opacity: 1, height: 'auto' }}
                                    className={`p-6 rounded-2xl flex flex-col gap-4 ${importStatus.type === 'success' ? 'bg-emerald-500/5 text-emerald-600 border border-emerald-500/20' : 'bg-rose-500/5 text-rose-600 border border-rose-500/20'}`}
                                >
                                    <div className="flex items-center gap-3 font-bold text-sm">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${importStatus.type === 'success' ? 'bg-emerald-500/20' : 'bg-rose-500/20'}`}>
                                    ...
                                    <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                        <motion.div
                                            initial={{ width: 0 }}
                                            animate={{ width: importStatus.type === 'success' ? '100%' : '45%' }}
                                            className={`h-full ${importStatus.type === 'success' ? 'bg-emerald-500' : 'bg-rose-500'}`}
                                        />
                                    </div>
                                </motion.div>
                            )}
                            ...
                            <div className="flex flex-col lg:flex-row gap-4 pt-4">
                                <button
                                    onClick={() => {
                                        setImportStatus({ type: 'success', msg: 'Sincronizando com o Núcleo IA: Gerando questões...' })
                                        setTimeout(() => {
                                            handleBatchImport()
                                        }, 1500)
                                    }}
                                    className="flex-1 royal-gradient text-white py-6 rounded-[24px] font-black uppercase text-xs tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-primary/25 flex items-center justify-center gap-3 group"
                                >
                                    <Sparkles className="w-5 h-5 group-hover:animate-pulse" />
                                    Executar Geração Massiva (Lote #{selectedBatchSize})
                                </button>
                                <button
                ...
                {view === 'validation' && (
                    <motion.div key="v" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                        <div className="flex items-center justify-between">
                            <h3 className="text-2xl font-black italic uppercase tracking-tighter">Fila de Validação</h3>
                            <button onClick={fetchRealData} className="p-2 hover:bg-muted rounded-full transition-colors">
                                <RefreshCw className="w-5 h-5 text-muted-foreground" />
                            </button>
                        </div>
                        
                        {pendingPackages.length === 0 ? (
                            <div className="bg-card border border-border rounded-[40px] p-10 text-center space-y-4">
                                <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto opacity-20" />
                                <h3 className="text-xl font-bold tracking-tight">Tudo em dia!</h3>
                                <p className="text-muted-foreground text-sm max-w-md mx-auto">Não há lotes de questões aguardando validação no momento.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 gap-6">
                                {pendingPackages.map(pkg => (
                                    <div key={pkg.id} className="bg-card border border-border/50 rounded-3xl p-6 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between hover:border-primary/30 transition-colors">
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-3">
                                                <span className="bg-primary/10 text-primary text-[10px] font-black px-2 py-1 rounded-lg border border-primary/20 uppercase tracking-tighter">
                                                    {pkg.banks?.name || 'IA GENERATED'}
                                                </span>
                                                <span className="bg-amber-500/10 text-amber-500 text-[10px] font-black px-2 py-1 rounded-lg border border-amber-500/20 uppercase tracking-tighter">
                                                    {pkg.status}
                                                </span>
                                            </div>
                                            <h4 className="text-lg font-black italic uppercase truncate max-w-md">{pkg.name}</h4>
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                                                <Layers className="w-3 h-3" /> {pkg.taxonomy_path || 'SEM TAXONOMIA'}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-12 shrink-0">
                                            <div className="text-center">
                                                ...
                                                <p className="text-2xl font-black italic">{pkg.package_questions?.[0]?.count || 0} Q</p>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <button 
                                                    disabled={isPublishing === pkg.id}
                                                    onClick={() => handlePublishPackage(pkg.id)}
                                                    className={`px-6 py-3 ${isPublishing === pkg.id ? 'bg-muted' : 'bg-[#1A1033]'} text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-[1.02] transition-transform active:scale-95 flex items-center gap-2`}
                                                >
                                                    {isPublishing === pkg.id ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                                                    {isPublishing === pkg.id ? 'Publicando...' : 'Aprovar Master'}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                ...
                {view === 'regulation' && (
                    <motion.div key="r" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                        <div className="bg-card border border-border rounded-[40px] p-10 text-center space-y-4">
                            ...
                            <h3 className="text-2xl font-black italic uppercase tracking-tighter">Regulação e Compliance</h3>
                            <p className="text-muted-foreground text-sm max-w-md mx-auto">Gestão de acessos, logs de sistema e configurações de segurança.</p>
                        </div>
                    </motion.div>
                )}

                {view === 'prompt' && (
                    <motion.div key="p-gen" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                        <div className="bg-card border border-border rounded-[40px] p-10 space-y-6">
                             <div className="flex items-center gap-4">
                                <div className="royal-gradient p-3 rounded-2xl text-white shadow-lg shrink-0"><Target className="w-6 h-6" /></div>
                                <div>
                                    <h3 className="text-2xl font-black italic uppercase tracking-tighter">Gerador de Prompt Master</h3>
                                    <p className="text-sm font-medium text-muted-foreground">Construa prompts otimizados para o Claude/GPT gerar questões no formato JSON.</p>
                                </div>
                            </div>
                            <div className="bg-muted rounded-2xl p-6 font-mono text-xs text-primary whitespace-pre-wrap">
                                {`Tu és um mestre em Medicina, especialista em concursos de residência e Revalida.
Gere questões seguindo estes critérios:
- Especialidade: ${activeSpecialty?.name || 'A definir'}
- Dificuldade: Alta
- Formato: Caso clínico denso
- Saída: JSON compatível com Master Mapping protocol.`}
                            </div>
                            <button className="w-full py-4 bg-[#1A1033] text-white rounded-2xl font-black uppercase text-xs tracking-widest">
                                Copiar Prompt Base
                            </button>
                        </div>
                    </motion.div>
                ...
                {view === 'taxonomy' && (
                    <motion.div key="tax" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                        <div className="bg-card border border-border rounded-[40px] p-10 text-center space-y-4">
                            <Layers className="w-12 h-12 text-primary mx-auto opacity-20" />
                            <h3 className="text-2xl font-black italic uppercase tracking-tighter">Arquitetura de Taxonomia</h3>
                            <p className="text-muted-foreground text-sm max-w-md mx-auto">Edição de Áreas, Especialidades e Temas do banco de Saúde.</p>
                        </div>
                    </motion.div>
                )}
                {view === 'analytics' && (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <StatCard
                                ...
                                value={dbStats.users.toLocaleString('pt-BR')}
                                sub="+12 hoje"
                                color="text-primary"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <section className="bg-card glass-card border border-border/50 rounded-[40px] p-8 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <GenStat label="Geradas Hoje" value={dbStats.pending.toLocaleString('pt-BR')} />
...
function StatCard({ label, value, sub, color, icon: Icon, alert }: { label: string, value: string | number, sub?: string, color: string, icon: any, alert?: boolean }) {
    return (
        <div className={`bg-white border ${alert ? 'border-rose-500/30 shadow-rose-500/10' : 'border-slate-100'} rounded-[32px] p-8 shadow-sm flex flex-col gap-4 relative overflow-hidden group hover:shadow-xl transition-all`}>
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${color.replace('text-', 'bg-')}/10 ${color}`}>
