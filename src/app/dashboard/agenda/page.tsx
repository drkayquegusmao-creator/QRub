...
export default function SaudeAgendaPage() {
    ...
    const markComplete = useCallback((taskId: string) => {
        ...
        setStats(prev => ({
            ...
            percentComplete: prev.total > 0 ? Math.round(((prev.completed + 1) / prev.total) * 100) : 0
        }))
    }, [])
        ...
        <div className="space-y-12 pb-32 animate-in fade-in duration-700">
            ...
            <div className="bg-[#111827] rounded-b-[40px] -mx-8 -mt-8 p-12 pt-16 flex flex-col md:flex-row items-center justify-between gap-12 relative overflow-hidden shadow-2xl">
                ...
                <div className="space-y-6 relative z-10 w-full md:w-auto">
                    <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase tracking-[0.2em]">
                        ...
                        {streak > 0 ? `${streak} dias de consistência` : 'Agenda Médica Diária'}
                    </div>
                    <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-6 relative z-10">
                        ...
                        <div className="relative w-32 h-32 flex items-center justify-center">
                            <svg className="w-full h-full transform -rotate-90">
                                ...
                                <motion.circle
                                    ...
                                    animate={{ strokeDashoffset: loading ? 364.4 : 364.4 - (364.4 * Math.min(stats.percentComplete, 100)) / 100 }}
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <MiniStat label="Concluído" value={loading ? '—' : `${stats.completed}/${stats.total}`} color="text-emerald-400" />
                            <MiniStat label="Atrasados" value={loading ? '—' : stats.late} color={stats.late > 0 ? 'text-amber-400' : 'text-slate-400'} />
                            <MiniStat label="Tempo Est." value={loading ? '—' : formatMinutes(stats.totalEstimatedMinutes)} color="text-blue-400" />
                        ...
                        <button
                            ...
                            className="w-full px-8 py-4 bg-emerald-600 text-white rounded-3xl font-black text-[11px] uppercase tracking-widest shadow-2xl shadow-emerald-600/30 hover:bg-emerald-500 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-40 disabled:cursor-not-allowed"
                        >
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-white/5">
                <div className="flex items-center gap-4">
                    <button className="p-3 bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-white/5 text-slate-400 hover:text-emerald-500 transition-all shadow-sm">
                        <ChevronLeft className="w-6 h-6" />
                    </button>
                    <div className="flex items-center gap-3 px-6 py-3 bg-white dark:bg-[#111827] rounded-3xl border border-slate-200 dark:border-white/5 shadow-sm">
                        <CalendarDays className="w-5 h-5 text-emerald-500" />
                        <span className="text-[11px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Hoje</span>
                ...
                <div className="flex items-center gap-2">
                    <button
                        onClick={loadData}
                        className="p-3 bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-white/5 text-slate-400 hover:text-emerald-500 transition-all shadow-sm"
                        title="Recarregar agenda"
                    >
                        <RefreshCw className={cn("w-4 h-4", loading && "animate-spin")} />
                    </button>
                    <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-[#111827] rounded-2xl border border-transparent dark:border-white/5">
                        <button
                            onClick={() => setView('daily')}
                            className={cn("px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", view === 'daily' ? "bg-white dark:bg-white/10 text-slate-900 dark:text-white shadow-sm" : "text-slate-400")}
                        >Diário</button>
                        <button
                            onClick={() => setView('weekly')}
                            className={cn("px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", view === 'weekly' ? "bg-white dark:bg-white/10 text-slate-900 dark:text-white shadow-sm" : "text-slate-400")}
                        >Semanal</button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-8 space-y-8">
                    <div className="flex items-center justify-between">
                        <h2 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white flex items-center gap-4">
                            <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500">
                                <Target className="w-6 h-6" />
                            </div>
                            Ordem de <span className="text-emerald-600 dark:text-emerald-400">Execução</span>
                        </h2>
                        {stats.late > 0 && (
                            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 text-[10px] font-black uppercase tracking-wider">
                    ...
                    {!loading && error && (
                        <div className="p-12 rounded-[32px] border-2 border-rose-500/20 bg-rose-500/5 flex flex-col items-center gap-6 text-center">
                            ...
                            <button onClick={loadData} className="px-8 py-3 bg-rose-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/20">Tentar novamente</button>
                        </div>
                    )}
                    ...
                    {!loading && !error && tasks.length > 0 && (
                        <div className="space-y-6 relative before:absolute before:left-[27px] before:top-8 before:bottom-8 before:w-1 before:bg-slate-200 dark:before:bg-white/5">
                            <AnimatePresence>
                            ...
                            <button className="w-full p-8 border-2 border-dashed border-slate-300 dark:border-white/10 rounded-[32px] flex items-center justify-center gap-4 text-slate-400 hover:text-emerald-500 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all group">
                ...
                <div className="lg:col-span-4 space-y-10">
                    ...
                    <div className="p-10 rounded-[40px] border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111827] shadow-xl">
                        <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white mb-10 flex items-center gap-4 leading-none">
                            <Calendar className="w-7 h-7 text-emerald-500" /> Atividade <span className="opacity-40">Semanal</span>
                        </h3>
                            ...
                            {(weeklyActivity.length > 0
                                ...
                                return (
                                    <div key={i} className="flex flex-col items-center gap-3">
                                        <span className={cn("text-[9px] font-black tracking-widest", isToday ? "text-emerald-500" : "text-slate-400")}>
                                            {dayLabel}
                                        </span>
                                        <div className={cn(
                                            ...
                                            isToday && intensity === 0 ? "bg-emerald-600/10 border-emerald-400/50 text-emerald-500" :
                                                isToday ? "bg-emerald-500 border-emerald-400 shadow-xl shadow-emerald-500/20 text-white" :
                                                    intensity === 3 ? "bg-blue-500/20 border-blue-500/30 text-blue-500" :
                                                        intensity === 2 ? "bg-emerald-500/15 border-emerald-500/20 text-emerald-500" :
                    ...
                    {(stats.late > 0 || loading) && (
                        <div className="p-10 bg-amber-500/5 border-2 border-amber-500/10 rounded-[40px] space-y-8">
                            <div className="flex items-center justify-between">
                                <h3 className="text-xl font-black italic uppercase tracking-tighter text-amber-600 flex items-center gap-4">
                                    <AlertCircle className="w-7 h-7" /> Pendências
                                </h3>
                                <div className="px-2 py-0.5 bg-amber-500 text-white text-[9px] font-black uppercase tracking-widest rounded-lg">
                            ...
                            <div className="space-y-4">
                                ...
                                {loading && (
                                    <div className="h-16 rounded-2xl bg-amber-500/10 animate-pulse" />
                                )}
                            </div>
                            <button className="w-full py-5 bg-amber-500 text-white rounded-[24px] text-[10px] font-black uppercase tracking-widest shadow-xl shadow-amber-500/20 hover:scale-[1.02] active:scale-95 transition-all">
                                Priorizar Atrasados
                            </button>
                        </div>
                    )}
                    ...
                    {/* Study Streak Card */}
                    <div className="p-10 rounded-[40px] border-none bg-gradient-to-br from-[#111827] to-[#1e293b] text-white shadow-2xl overflow-hidden relative">
                        <TrendingUp className="absolute -right-8 -bottom-8 w-48 h-48 opacity-10 pointer-events-none text-emerald-500" />
                        <div className="relative z-10 space-y-6">
                            <div className="flex items-start justify-between">
                                <div className="space-y-1">
                                    <p className="text-[9px] font-black uppercase tracking-[0.4em] text-emerald-400 opacity-80">Sequência Ativa</p>
                                    ...
                                    return (
                                        <div key={i} className="flex-1 rounded-t-lg bg-white/10 relative group">
                                            <motion.div
                                                ...
                                                className="absolute bottom-0 w-full bg-emerald-500 rounded-t-lg opacity-40 group-hover:opacity-100 transition-opacity"
                                            />
                                        </div>
                                    )
                                })}
                            </div>
                            <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Estatísticas Vitais dos Últimos 7 dias</p>
                        </div>
                    </div>
    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                isLate ? "bg-amber-500/5 border-amber-500/20" : "bg-white dark:bg-[#111827] border-slate-200 dark:border-white/5 shadow-sm",
                isDone && "grayscale opacity-50 scale-[0.98]",
                "hover:border-emerald-500/30 hover:scale-[1.01] hover:shadow-2xl active:scale-[0.99]"
            )}
        >
            {/* Icon pillar */}
                ...
                <div className={cn(
                    ...
                    isDone ? "bg-emerald-500 text-white" :
                        isLate ? "bg-amber-500 text-white" :
                            `${bg} ${color} group-hover:bg-emerald-500 group-hover:text-white`
                )}>
            ...
            <div className="flex-1 space-y-5">
                <div className="flex flex-wrap items-center gap-3">
                    <span className={cn("text-[10px] font-black uppercase tracking-[0.2em] italic", isLate ? "text-amber-600" : color)}>
                        {label}
                    </span>
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-200 dark:bg-white/10" />
                    <span className="text-[11px] font-black uppercase tracking-widest text-[#111827] dark:text-white truncate">
                        {task.discipline}
                    </span>
                ...
                <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tighter text-[#111827] dark:text-white leading-tight">
                    {task.subject}
                </h3>
                ...
                <div className="flex flex-wrap items-center gap-8">
                    {task.quantity && (
                        <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                            <Target className="w-4 h-4 text-emerald-500 opacity-50" /> {task.quantity}
                        </div>
                    )}
                    <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                        <Timer className="w-4 h-4 text-emerald-500 opacity-50" /> {task.estimatedTime}
                    </div>
                    <div className={cn(
                ...
                {!isDone && (
                    <div className="flex items-center gap-4 pt-4">
                        <button
                            onClick={onComplete}
                            className="flex-1 md:flex-none px-8 py-4 bg-[#111827] dark:bg-white text-white dark:text-[#111827] rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:translate-y-[-2px] active:translate-y-0 transition-all flex items-center justify-center gap-3"
                        >
                            Concluir <CheckCircle2 className="w-4 h-4" />
                        </button>
                        <button className="px-6 py-4 bg-slate-50 dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-emerald-500 rounded-2xl transition-all">
                            Adiar
                        </button>
                        <button className="p-4 text-slate-300 hover:text-emerald-500 transition-colors ml-auto">
            ...
            {isLate && (
                <div className="absolute top-0 right-0 overflow-hidden rounded-tr-[30px]">
                    <div className="bg-amber-500 text-white text-[9px] font-black uppercase px-6 py-1.5 rotate-45 translate-x-4 translate-y-2 shadow-lg">Atrasado</div>
                </div>
            )}
        </motion.div>
    )
...
function PendingItem({ title, delay, type }: { title: string; delay: string; type: 'revisao' | 'questoes' }) {
    ...
    return (
        <div className="flex items-center gap-4 p-5 bg-white dark:bg-white/5 border border-amber-500/20 rounded-3xl group cursor-pointer hover:bg-amber-500/10 transition-all">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center text-amber-500">
                <Icon className="w-5 h-5" />
            </div>
            <div className="flex-1 space-y-0.5 min-w-0">
                <p className="text-[11px] font-black uppercase text-slate-900 dark:text-white tracking-tight truncate">{title}</p>
                <p className="text-[8px] font-bold text-amber-600 uppercase tracking-widest">{delay}</p>
            </div>
            <button className="p-2.5 rounded-xl bg-amber-500 text-white opacity-0 group-hover:opacity-100 transition-all">
...
function TasksSkeleton() {
    return (
        <div className="space-y-6">
            {[1, 2, 3].map(i => (
                <div key={i} className="p-8 rounded-[32px] bg-slate-100 dark:bg-white/5 animate-pulse">
...
function EmptyAgenda() {
    return (
        <motion.div
            ...
            className="p-16 rounded-[40px] border-2 border-dashed border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-transparent flex flex-col items-center gap-8 text-center"
        >
            ...
            <div className="space-y-3">
                <h3 className="text-3xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">
                    Agenda <span className="text-emerald-500">Zero!</span>
                </h3>
                <p className="text-slate-500 dark:text-slate-400 font-medium text-sm max-w-xs">
                    Suas atividades de hoje foram todas finalizadas. Você está dominando a rotina.
                </p>
            </div>
            <div className="flex items-center gap-4 mt-4">
                <button className="px-8 py-4 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-500/20 hover:scale-105 transition-all">
                    Aprofundar Tópico Aleatório
                </button>
            </div>
        </motion.div>
    )
...
function FocusModeView({
    ...
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="fixed inset-0 bg-[#0B0F1A] z-[100] flex flex-col items-center justify-center p-12 overflow-hidden"
        >
            ...
            <button
                onClick={onClose}
                className="absolute top-12 right-12 p-5 text-white/30 hover:text-white transition-all bg-white/5 rounded-full hover:scale-110 active:scale-90 z-20"
            >
            ...
            <div className="w-full max-w-4xl space-y-16 text-center relative z-10">
                <div className="space-y-6">
                    <motion.div
                        ...
                        animate={{ scale: 1, y: 0 }}
                        className="w-32 h-32 rounded-[40px] bg-emerald-500 flex items-center justify-center mx-auto shadow-[0_30px_60px_-15px_rgba(16,185,129,0.5)]"
                    >
                        <Icon className="w-16 h-16 text-white" />
                    </motion.div>
                    <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase tracking-[0.3em]">
                        Missão Médica em Andamento
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <p className="text-emerald-400 text-xl md:text-2xl font-black italic uppercase tracking-[0.2em]">
                        {task.discipline}
                    </p>
                </div>
                ...
                <div className="flex items-center justify-center gap-16 text-white/20">
                    <div className="flex flex-col items-center gap-3 group">
                        <Clock className="w-8 h-8 group-hover:text-emerald-400 transition-colors" />
                    ...
                    {task.quantity && (
                        <>
                            ...
                            <div className="flex flex-col items-center gap-3 group">
                                <Target className="w-8 h-8 group-hover:text-emerald-400 transition-colors" />
                ...
                <div className="pt-12 flex flex-col md:flex-row items-center justify-center gap-8">
                    <button
                        ...
                        className="w-full md:w-auto px-16 py-8 bg-white text-slate-900 rounded-[32px] font-black text-xl uppercase tracking-widest shadow-[0_25px_50px_-12px_rgba(255,255,255,0.25)] hover:bg-emerald-50 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 group"
                    >
            ...
            <div className="absolute bottom-0 left-0 w-full h-2 bg-white/5">
                <motion.div
                    ...
                    className="h-full bg-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.8)]"
                />
            </div>
        </motion.div>
    )
