...
import { 
    getConcursoBanks, createConcursoBank, updateConcursoBank,
    getConcursoBankProfiles, createConcursoBankProfile, setConcursoCurrentProfile,
    getConcursoBlueprints, createConcursoBlueprint, updateConcursoBlueprint,
    type ConcursoBank, type ConcursoBankProfile, type ConcursoQuestionBlueprint
...
function BankProfileForm({ bankId, onSaved }: { bankId: string; onSaved: () => void }) {
    ...
    async function handleSave() {
        ...
        try {
            ...
            const { error: err } = await createConcursoBankProfile({
                bank_id: bankId,
    ...
    return (
        <div className="space-y-6 bg-slate-50 border border-slate-200 rounded-[2.5rem] p-8">
            ...
            <div className="flex items-center gap-6 pt-2">
                ...
                >
                    {loading ? 'Salvando...' : '💾 Salvar Perfil Master'}
                </button>
            </div>
        </div>
    )
...
function BlueprintForm({ bankId, onSaved }: { bankId: string; onSaved: () => void }) {
    const [form, setForm] = useState({ name: '', format: 'mcq_5' as ConcursoQuestionBlueprint['format'], description: '', blueprint_rules: '{}' })
    const [loading, setLoading] = useState(false)
    const [error, setError] = useState<string | null>(null)
    ...
    async function handleSave() {
        ...
        try {
            ...
            const { error: err } = await createConcursoBlueprint({
                bank_id: bankId,
                name: form.name,
                format: form.format,
        ...
        <div className="space-y-6 bg-slate-50 border border-slate-200 rounded-[2.5rem] p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="space-y-2">
                    ...
                    <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                        placeholder="Ex: Padrão FGV" className="w-full bg-white border border-slate-200 rounded-2xl px-5 py-3 text-slate-800 text-sm focus:outline-none focus:border-indigo-500 shadow-sm font-bold" />
                </div>
                <div className="space-y-2">
                    <label className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1.5 block">Formato Estrutural</label>
                    <select value={form.format} onChange={e => setForm(p => ({ ...p, format: e.target.value as ConcursoQuestionBlueprint['format'] }))}
                ...
                <div className="space-y-2">
                    ...
                    <input type="text" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                        placeholder="Ex: Questão de Interpretação" className="w-full bg-white border border-slate-200 rounded-2xl px-5 py-3 text-slate-800 text-sm focus:outline-none focus:border-indigo-500 shadow-sm font-bold" />
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <textarea value={form.blueprint_rules} onChange={e => setForm(p => ({ ...p, blueprint_rules: e.target.value }))}
                    rows={3} placeholder='{"focus": "norma culta"}' className="w-full bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4 text-emerald-400 text-xs font-mono focus:outline-none shadow-lg resize-none" />
            </div>
...
function BankDetailPanel({ bank, onClose }: { bank: ConcursoBank; onClose: () => void }) {
    const [tab, setTab] = useState<'perfis' | 'blueprints'>('perfis')
    const [profiles, setProfiles] = useState<ConcursoBankProfile[]>([])
    const [blueprints, setBlueprints] = useState<ConcursoQuestionBlueprint[]>([])
    const [loading, setLoading] = useState(true)

    const load = useCallback(async () => {
        setLoading(true)
        const [p, b] = await Promise.all([getConcursoBankProfiles(bank.id), getConcursoBlueprints(bank.id)])
        setProfiles(p.data)
    ...
    async function handleSetCurrent(profileId: string) {
        await setConcursoCurrentProfile(profileId, bank.id)
        load()
        toast.success('Perfil ativado como atual')
    }

    async function handleToggleBlueprint(bp: ConcursoQuestionBlueprint) {
        await updateConcursoBlueprint(bp.id, { is_active: !bp.is_active })
        load()
    }
                ...
                <div className="flex border-b border-slate-100 bg-white px-10">
                    {(['perfis', 'blueprints'] as const).map(t => (
                        <button key={t} onClick={() => setTab(t)}
                            ...
                            {t === 'perfis' ? '🧬 Perfis de Estilo' : '🔷 Blueprints Concursos'}
                        ...
                        <>
                            ...
                            {profiles.length === 0 ? (
                                <div className="text-center py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem]">
                                    ...
                                    <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">Nenhuma inteligência cadastrada</p>
                                </div>
                            ) : (
                        ...
                        <>
                            ...
                            {blueprints.length === 0 ? (
                                <div className="text-center py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem]">
                                    ...
                                    <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">Nenhum formato estrutural definido</p>
                                </div>
                            ) : (
...
export default function ConcursoBancasPage() {
    ...
    const loadBanks = useCallback(async () => {
        setLoading(true)
        const { data, error: err } = await getConcursoBanks()
    ...
    async function handleToggleActive(bank: ConcursoBank) {
        await updateConcursoBank(bank.id, { is_active: !bank.is_active })
        loadBanks()
    }
    ...
    return (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                    <h1 className="text-5xl font-black italic uppercase tracking-tighter text-slate-900 flex items-center gap-5 leading-none">
                    ...
                    <p className="text-slate-400 text-[11px] font-black uppercase tracking-[0.4em] mt-3 ml-1 flex items-center gap-2">
                        <ShieldCheck size={14} className="text-indigo-500" />
                        Ambiente Concursos Ativo
                    </p>
                </div>
                <button
                    onClick={() => setIsCreateModalOpen(true)}
                    className="flex items-center gap-4 px-12 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2.5rem] font-black uppercase text-xs tracking-widest shadow-2xl shadow-indigo-600/20 hover:scale-105 active:scale-95 transition-all"
                >
                    <Plus size={20} />
                    Cadastrar Banca
                </button>
            ...
            {/* Global Filters */}
            <div className="flex bg-white border border-slate-100 rounded-[2.5rem] p-6 items-center gap-6 max-w-xl shadow-sm">
                <Search className="text-slate-300" size={24} />
                <input 
                    placeholder="Buscar por nome da banca examinadora..."
                    value={searchTerm}
                    onChange={e => setSearchTerm(e.target.value)}
                    className="bg-transparent border-none outline-none font-bold text-base w-full placeholder:text-slate-300"
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {loading ? (
                    ...
                    <div className="lg:col-span-3 py-32 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[4rem] flex flex-col items-center justify-center text-center">
                        <Building2 size={64} className="text-slate-200 mb-6" />
                        <h3 className="text-xl font-black uppercase text-slate-400">Nenhuma banca encontrada</h3>
                        <p className="text-xs text-slate-300 font-bold uppercase tracking-widest mt-2">Inicie o cadastro para o ecossistema concursos.</p>
                    </div>
                ) : filteredBanks.map(bank => (
                    <div
                        key={bank.id}
                        className={`group bg-white border rounded-[3.5rem] p-10 transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(79,70,229,0.15)] hover:-translate-y-2 ${bank.is_active ? 'border-slate-100' : 'border-slate-50 opacity-60 grayscale'}`}
                    >
                        <div className="flex items-start justify-between gap-6 mb-8">
                            <div className="space-y-3">
                                <h3 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900 group-hover:text-indigo-600 transition-colors leading-tight">{bank.name}</h3>
                                <div className="flex items-center gap-3">
                                    <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${bank.is_active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                        {bank.is_active ? 'Live' : 'Pausada'}
                                    </span>
                                    <p className="text-slate-300 text-[9px] font-mono tracking-tighter uppercase">{bank.slug}</p>
                                </div>
                            </div>
                            <div className="w-16 h-16 rounded-[2rem] bg-slate-50 border border-slate-50 flex items-center justify-center text-3xl group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                                🏛️
                            </div>
                        </div>

                        {bank.description && <p className="text-slate-500 text-xs font-medium leading-relaxed mb-8 line-clamp-2 italic">&quot;{bank.description}&quot;</p>}

                        <div className="flex items-center gap-4 pt-8 border-t border-slate-50">
                            <button
                                onClick={() => setSelectedBank(bank)}
                                className="flex-1 py-4 bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white text-[10px] font-black uppercase tracking-widest rounded-2xl transition-all shadow-sm hover:shadow-xl hover:shadow-indigo-600/20 active:scale-95"
                            >
                                Configurar IA
                            </button>
                            <button
                            ...
                            >
                                {bank.is_active ? <XCircle size={20} /> : <CheckCircle2 size={20} />}
                            </button>
                        </div>
                    </div>
                ))}
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-xl rounded-[3rem] p-12 space-y-10 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="space-y-6 relative">
                                <div className="space-y-2">
                                    ...
                                    <input type="text" placeholder="Ex: CEBRASPE, FGV, VUNESP" value={newBank.name}
                                        onChange={e => setNewBank(p => ({ ...p, name: e.target.value }))}
                                        className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 text-slate-900 text-sm font-bold focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Slug (Link)</label>
                                    <input type="text" placeholder="Ex: fgv-concursos" value={newBank.slug}
                                        onChange={e => setNewBank(p => ({ ...p, slug: e.target.value }))}
                                        className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 text-slate-900 text-xs font-mono focus:outline-none focus:border-indigo-500 transition-all" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Especialidade / Perfil</label>
                                    <textarea placeholder="Ex: Focada em questões discursivas e casuais..." value={newBank.description}
                                        onChange={e => setNewBank(p => ({ ...p, description: e.target.value }))}
                                        className="w-full h-32 bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 text-slate-900 text-sm font-medium focus:outline-none focus:border-indigo-500 transition-all resize-none" />
                                </div>
                            </div>
