...
for (let batchNum = 1; batchNum <= 10; batchNum++) {
    ...
    questions.forEach((q, i) => {
        ...
        const options = JSON.stringify(q.options ? Object.entries(q.options).map(([k, v]) => ({ id: k, text: v })) : []);
        ...
        sql += `        ${escapeSql(q.enunciado)}, ${escapeSql(options)}, ${escapeSql(q.answer)}, \n`;
...
console.log('SQL generated to import_all_cirrose.sql');
