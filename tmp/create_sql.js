...
const StringUtilComputeHash = (text) => {
...
const encodeSQLString = (str) => {
    return "'" + String(str).replace(/'/g, "''") + "'";
};

let sql = `-- Inserting 15 generated questions into package_questions for package ID ${PACKAGE_ID}\n\n`;

for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const hash = StringUtilComputeHash(q.enunciado);

    // We do ON CONFLICT to avoid duplicate key errors, or just insert
    // Checking hash logic inside DB is tricky unless it's a unique constraint, but we can just use do nothing if unique is there, otherwise we just insert directly.
    // If they don't have unique constraint, just insert.
    sql += `INSERT INTO package_questions (package_id, question_json, hash_logico, order_index, status)
VALUES (
  '${PACKAGE_ID}',
  ${encodeSQLString(JSON.stringify(q))}::jsonb,
  '${hash}',
  ${i},
  'draft'
);\n\n`;
...
fs.writeFileSync('tmp/inject_questions_raw.sql', sql, 'utf8');
console.log('Saved to tmp/inject_questions_raw.sql');
