...
const fs = require('fs');
const PACKAGE_ID = '44BB9F70-13D0-42E0-808E-8DED933CEA6A';
...
for (let batchNum = 1; batchNum <= 4; batchNum++) {
    const fileName = 'batch_fgv_tuberculose_part' + batchNum + '.json';
    if (!fs.existsSync(fileName)) {
        console.log(`Aviso: Arquivo ${fileName} não encontrado.`);
        continue;
    }
    ...
    questions.forEach((q, i) => {
        ...
        sql += '    -- Q' + globalCounter + ' (Batch ' + batchNum + ')\n';
        ...
        sql += '    ) VALUES (\n';
        sql += '        ' + escapeSql(questionId) + ', \'medicina\', \'clinica-medica\', \'infectologia\', \'infectologia\', \'tuberculose\', \'tuberculose\',\n';
        ...
        sql += '    VALUES (p_id, ' + escapeSql(questionId) + ', \'approved\', ' + (globalCounter - 1) + ')\n';
...
fs.writeFileSync('import_all_tuberculose.sql', sql);
console.log(`SQL gerado com sucesso para ${globalCounter - 1} questões de Tuberculose.`);
